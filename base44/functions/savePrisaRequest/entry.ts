...
export default async function(req) {
  try {
    ...
    const current = id ? await base44.asServiceRole.entities.PrisaRequest.get(id).catch(() => null) : null;
    if (id && !current) return Response.json({ success: false, error: 'בקשת הפריסה לא נמצאה' }, { status: 404 });
    ...
    const item = id
      ? await base44.asServiceRole.entities.PrisaRequest.update(id, payload)
      : await base44.asServiceRole.entities.PrisaRequest.create(payload);
    return Response.json({ success: true, item });
  } catch (error) {
    return Response.json({ success: false, error: error.message || 'שמירת הפריסה נכשלה' }, { status: 500 });
  }
}