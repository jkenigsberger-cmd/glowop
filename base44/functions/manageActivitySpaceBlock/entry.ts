...
async function findConflicts(base44, input) {
  ...
  const matchingItems = items.filter(i => i.date >= input.start_date && i.date <= input.end_date && overlaps(input.start_time, input.end_time, i.start_time, i.end_time));
  const matchingRequests = activeRequests.filter(r => r.date >= input.start_date && r.date <= input.end_date && overlaps(input.start_time, input.end_time, r.start_time, r.end_time));
...
Deno.serve(async (req) => {
  try {
    ...
    if (action === 'cancel') {
      ...
      const block = await base44.asServiceRole.entities.ActivitySpaceBlock.update(body.id, { status: 'CANCELLED', cancelled_by: user.email, cancelled_at: new Date().toISOString() });
    ...
    if (!input.activity_space_id || !input.start_date || !input.end_date || !input.start_time || !input.end_time || !REASONS.has(input.reason_type)) return Response.json({ success: false, error: 'חסרים פרטי חסימה' }, { status: 400 });
    if (input.start_date > input.end_date || input.start_time >= input.end_time) return Response.json({ success: false, error: 'טווח התאריכים או השעות אינו תקין' }, { status: 400 });
    ...
    const payload = { activity_space_id: space.id, activity_space_name: space.name, start_date: input.start_date, end_date: input.end_date, start_time: input.start_time, end_time: input.end_time, reason_type: input.reason_type, reason_notes: input.reason_notes || '', status: 'ACTIVE', created_from_maintenance_issue_id: input.created_from_maintenance_issue_id || null };
