...
export default function PreparationGroupCard({ group, quote, profile, canActivate, onActivated }) {
  return <div className="bg-card border border-violet-200 rounded-xl p-4 space-y-2" dir="rtl">
    ...
    <div className="flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link to={`/groups/${group.id}${profile ? "#operational-profile" : ""}`}>{profile ? "פתח פרופיל תפעולי" : "פתח קבוצה"}</Link></Button>{canActivate && <OperationalActivationAction groupId={group.id} onActivated={onActivated} />}</div>
  </div>;
}