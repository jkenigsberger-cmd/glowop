...
export default function StaffGenderSplitEditor({ total, men, women, onChange }) {
  ...
  const setMen = value => {
    ...
    const nextMen = Math.max(0, Math.min(Math.trunc(parsed), totalValue));
  ...
  const setWomen = value => {
    ...
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) return;
    const nextWomen = Math.max(0, Math.min(Math.trunc(parsed), totalValue));
    ...
    <div className="space-y-2 mt-1">
      <div className="grid grid-cols-2 gap-2">
        <label className="rounded-lg border border-emerald-300 bg-emerald-100 px-2 py-1.5 text-center">
          ...
          <Input type="number" min="0" max={totalValue} step="1" inputMode="numeric" value={menValue} onChange={e => setMen(e.target.value)} className="h-7 border-0 bg-transparent p-0 text-center text-base font-bold text-emerald-800 shadow-none" />
        </label>
        <label className="rounded-lg border border-orange-300 bg-orange-100 px-2 py-1.5 text-center">
          ...
          <Input type="number" min="0" max={totalValue} step="1" inputMode="numeric" value={womenValue} onChange={e => setWomen(e.target.value)} className="h-7 border-0 bg-transparent p-0 text-center text-base font-bold text-orange-800 shadow-none" />
        </label>
      </div>
