...
/** Suggest smallest set of neighborhoods to cover required tent count */
function suggestNeighborhoods(neighborhoods, allTents, neededTents) {
  if (!neededTents || neededTents <= 0) return [];
  // sort by capacity desc so we fill larger ones first
...
export default function SleepingAllocationTab({ groupId }) {
  ...
  // All CONFIRMED SleepingAllocations from OTHER groups (VIP conflict check)
  ...
  // Other groups' reservations that overlap with our dates
  const otherNhoodResByNeighborhood = useMemo(() => {
    const map = {}; // neighborhood_id → { group_name, gender_group }
    ...
    allNhoodReservations.forEach(r => {
      ...
      const gName = groupById[r.group_id]?.group_name || r.group_id;
      map[r.neighborhood_id] = { group_name: gName, gender_group: r.gender_group };
    });
    return map;
  }, [allNhoodReservations, groupId, arrivalDate, departureDate, groupById]);

  // ── VIP conflict map (tent-level, from SleepingAllocations) ───────────────
  ...
  // My VIP allocations map by tent_id (non-cancelled)
  ...
  // My draft VIP conflict tent ids
  ...
  // ── Neighborhood reservation handlers ─────────────────────────────────────
  ...
  return (
    <div className="space-y-6" dir="rtl">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 space-y-1.5">
            ...
            {suggestion.map(({ neighborhood, tents, use, spare }) => (
              <div key={neighborhood.id} className="text-xs text-amber-700 flex items-center gap-2">
                ...
                {spare > 0 && (
                  <span className="text-amber-500 text-[10px]">({spare} אוהלים פנויים חסומים בגלל בלעדיות)</span>
                )}
              </div>
            ))}
      ...
      <section className="space-y-3">
        ...
        <p className="text-[11px] text-slate-500">
          אוהלי VIP (80–89) — קבוצות שונות יכולות להשתמש באוהלי VIP שונים בתאריכים חופפים. אוהל בודד לא יכול לחפוף.
        </p>
        ...
        {vipNeighborhoods.map(hood => {
          ...
          const conflictMap = {};
          hoodTents.forEach(t => {
            if (vipTentConflictMap[t.id]) conflictMap[t.id] = vipTentConflictMap[t.id];
          });
          return (
            <div key={hood.id}>
              <NeighborhoodInventoryPanel
                ...
                currentGroupAllocMap={myVipAllocByTent}
      ...
      {/* Tent allocation modal (VIP only) */}
      {allocateTentTarget && (
        <TentAllocationModal
