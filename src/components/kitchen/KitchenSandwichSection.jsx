...
export default function KitchenSandwichSection({ meals, groupMap, profileMap }) {
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between rounded-xl border border-orange-200 bg-orange-100 px-4 py-2.5 text-orange-800">
        <div className="flex items-center gap-2 text-base font-bold">
          <Sandwich className="h-5 w-5" />
          סנדוויצ&apos;ים
        </div>
        <div className="flex items-center gap-3 text-sm font-medium">
          <span>{meals.length} קבוצות</span>
          <span className="opacity-60">·</span>
          <span>{totalPax} מנות</span>
        </div>
      </div>
