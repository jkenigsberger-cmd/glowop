...
export default function MonthlyWorkerRow({ worker }) {
  ...
  return <>
    ...
    {open && <tr className="border-b border-slate-200 bg-slate-50"><td colSpan="7" className="p-3"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{worker.details.map((item, index) => <div key={`${item.date}-${item.row_type}-${index}`} className="rounded-lg border bg-white px-3 py-2 text-xs"><strong>{fmtReportDate(item.date)}</strong> · {item.row_label}<div className="text-slate-500">{item.is_night_on_call ? "לינה במקום" : `${item.start_time || "—"}–${item.end_time || "—"} · ${fmtHours(item.estimated_hours)} שעות`}</div></div>)}</div></td></tr>}
  </>;
}