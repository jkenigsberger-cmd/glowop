...
import { Calendar, Users, FileText, Pencil, CheckCircle, XCircle, Copy } from "lucide-react";
...
export default function QuoteCenterCard({ quote, group, profile, canDecide, onEdit, onApprove, onReject, onDuplicate }) {
  ...
  return <div className="bg-card border border-border rounded-xl p-4 space-y-3" dir="rtl">
    ...
    <div className="flex flex-wrap gap-2">{group && <Button asChild size="sm" variant="outline"><Link to={`/groups/${group.id}`}><FileText className="w-3.5 h-3.5" />פתח קבוצה</Link></Button>}<QuotePdfButton quote={quote} group={group} />{onDuplicate && <Button size="sm" variant="ghost" onClick={onDuplicate}><Copy className="w-3.5 h-3.5" />שכפול</Button>}<Button size="sm" variant="ghost" onClick={onEdit}><Pencil className="w-3.5 h-3.5" />עריכה</Button>{canDecide && ["DRAFT","SENT"].includes(quote.status) && <><Button size="sm" onClick={() => quote.multi_option_enabled ? setOptionDialogOpen(true) : onApprove("A")}><CheckCircle className="w-3.5 h-3.5" />אישור</Button><Button size="sm" variant="destructive" onClick={onReject}><XCircle className="w-3.5 h-3.5" />דחייה</Button></>}</div>
    <QuoteOptionApprovalDialog quote={quote} open={optionDialogOpen} onClose={() => setOptionDialogOpen(false)} onConfirm={(key) => { setOptionDialogOpen(false); onApprove(key); }} />
  </div>;
}