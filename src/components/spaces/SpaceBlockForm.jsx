...
const empty = { activity_space_id: "", start_date: "", end_date: "", start_time: "08:00", end_time: "18:00", is_open_ended: false, reason_type: "MAINTENANCE", reason_notes: "" };
  ...
  return (
    <form onSubmit={event => { event.preventDefault(); onSubmit(form); }} className="border border-slate-200 rounded-xl bg-white p-4 space-y-4" dir="rtl">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        {!form.is_open_ended && <label className="text-sm space-y-1"><span>תאריך סיום</span><input required type="date" value={form.end_date || ""} onChange={e => set("end_date", e.target.value)} className="w-full border rounded-lg px-3 py-2" /></label>}
        ...
        {!form.is_open_ended && <label className="text-sm space-y-1"><span>שעת סיום</span><input required type="time" value={form.end_time || ""} onChange={e => set("end_time", e.target.value)} className="w-full border rounded-lg px-3 py-2" /></label>}
      ...
      <label className="flex items-center gap-2 text-sm font-medium cursor-pointer w-fit"><input type="checkbox" checked={!!form.is_open_ended} onChange={e => setForm(current => ({ ...current, is_open_ended: e.target.checked, end_date: e.target.checked ? "" : current.end_date, end_time: e.target.checked ? "" : current.end_time }))} /><span>חסום עד תיקון / ללא תאריך סיום</span></label>
      <p className="text-xs text-slate-500">{form.is_open_ended ? "ללא תאריך סיום" : "חסימה עם תאריך סיום"}</p>
