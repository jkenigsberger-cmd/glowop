...
export default function SpaceBlockList({ blocks, canManage, onEdit, onCancel, onConflicts }) {
  ...
  return <div className="space-y-2" dir="rtl">{blocks.map(block => (
    <div key={block.id} className={`border rounded-xl px-4 py-3 flex gap-3 items-center flex-wrap ${block.status === "ACTIVE" ? "bg-amber-50 border-amber-200" : "bg-slate-50 border-slate-200 opacity-70"}`}>
      ...
      <div className="flex-1 min-w-[220px]"><p className="font-semibold text-sm">{block.activity_space_name} — {BLOCK_REASON_LABELS[block.reason_type] || block.reason_type}</p><p className="text-xs text-slate-500">{block.is_open_ended ? `${block.start_date} · מ-${block.start_time} — עד תיקון` : `${block.start_date} עד ${block.end_date} · ${block.start_time}–${block.end_time}`}</p>{block.reason_notes && <p className="text-xs text-slate-600 mt-1">{block.reason_notes}</p>}{block.resolution_notes && <p className="text-xs text-emerald-700 mt-1">סיבת שחרור / הערת סיום: {block.resolution_notes}</p>}</div>
      ...
      {block.status === "ACTIVE" && canManage && <><Button size="sm" variant="outline" onClick={() => onEdit(block)}><Pencil className="w-3.5 h-3.5" /> ערוך</Button><Button size="sm" variant="outline" className="text-emerald-700 border-emerald-200" onClick={() => onCancel(block)}><Ban className="w-3.5 h-3.5" /> שחרור חסימה</Button></>}
