...
export default function ActivitySpaceBlocksPanel({ spaces, blocks, role }) {
  ...
  const confirmSave = async () => { setSaving(true); await invoke({ action: "save", id: conflicts.pending.id, block: conflicts.pending.form, confirm_conflicts: true }); setConflicts(null); setEditing(null); await refresh(); setSaving(false); };
  const cancel = async block => { if (!window.confirm("לבטל את החסימה? ההיסטוריה תישמר.")) return; await invoke({ action: "cancel", id: block.id }); await refresh(); };
  const showConflicts = async block => { const result = await invoke({ action: "list_conflicts", block }); setConflicts({ rows: result.conflicts || [] }); };
  return <div className="space-y-4" dir="rtl">
    <div className="flex justify-between items-center"><div><h2 className="font-semibold">ניהול חסימות מרחבים</h2><p className="text-xs text-slate-500">חסימות פיזיות זמניות שאינן פעילויות קבוצה</p></div>{canManage && !editing && <Button onClick={() => setEditing({})}><Plus className="w-4 h-4" /> חסום מרחב</Button>}</div>
