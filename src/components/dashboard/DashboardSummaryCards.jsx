...
export default function DashboardSummaryCards({ stats }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      ...
      <Card icon={LogIn}          label="מגיעים היום"        value={stats.arrivingToday}     color="text-emerald-600" />
      <Card icon={Moon}           label="לנים הלילה"         value={stats.sleepingTonight}   color="text-indigo-600" />
      <Card icon={LogOut}         label="עוזבים היום"        value={stats.departingToday}    color="text-orange-600" />
      <Card icon={UserCheck}      label='סה"כ אנשים באתר'    value={stats.totalPaxOnSite}    color="text-slate-700" />
      <Card icon={UtensilsCrossed} label="ארוחות היום"       value={stats.mealsToday}        color="text-amber-600" />
      <Card icon={CalendarDays}   label="פעילויות היום"      value={stats.activitiesToday}   color="text-violet-600" />
