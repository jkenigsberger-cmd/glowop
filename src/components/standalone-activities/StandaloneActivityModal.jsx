...
export default function StandaloneActivityModal({ reservation, assignments = [], spaces, canDelete, onChanged, onClose }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await base44.functions.invoke("saveStandaloneActivityReservation", { ...form, id: reservation?.id, expected_pax: Number(form.expected_pax || 0), creation_token: creationToken, assignments: rows });
      toast.success("הפעילות נשמרה בהצלחה");
      await onChanged();
      onClose();
    } catch (caught) {
      const data = caught?.response?.data || {};
      setError(data.error === "SPACE_CONFLICT" ? `המרחב כבר תפוס בשעה שנבחרה${data.conflicting_title ? ` — ${data.conflicting_title} (${data.start_time}–${data.end_time})` : ""}` : "שמירת הפעילות נכשלה");
  ...
  const cancelReservation = async () => {
    ...
    setSaving(true);
    await base44.functions.invoke("cancelStandaloneActivityReservation", { id: reservation.id, reason });
    await onChanged();
  ...
  const deleteReservation = async () => {
    ...
    setSaving(true);
    await base44.functions.invoke("deleteStandaloneActivityReservation", { id: reservation.id });
    await onChanged();
    onClose();
  };
  ...
  return <Dialog open onOpenChange={onClose}><DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto" dir="rtl"><DialogHeader><DialogTitle>{reservation ? "פרטי פעילות כללית" : "פעילות כללית ללא קבוצה"}</DialogTitle></DialogHeader>
    ...
    <div className="space-y-3"><div className="flex justify-between items-center"><Label>בחירת מרחבים וציוד נדרש *</Label><Button type="button" size="sm" variant="outline" onClick={() => setRows((current) => [...current, emptyAssignment()])}><Plus className="w-4 h-4" /> הוסף מרחב</Button></div>{rows.map((row,index) => <StandaloneSpaceRow key={index} row={row} index={index} spaces={spaces} onChange={(patch) => updateRow(index, patch)} onRemove={() => setRows((current) => current.filter((_,i) => i !== index))} />)}</div>
