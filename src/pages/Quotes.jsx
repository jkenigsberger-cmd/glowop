...
export default function Quotes() {
  ...
  const decide = async (quote, action) => { if (action === "reject") { const reason = window.prompt("סיבת דחייה"); if (!reason) return; if (quote.preparation_flow_enabled) await base44.functions.invoke("rejectQuotePreparation", { quote_id: quote.id, rejection_reason: reason }); else await base44.entities.Quote.update(quote.id, { status: "REJECTED" }); } else { const functionName = quote.preparation_flow_enabled ? "approveQuoteAndActivateGroup" : "approveQuoteAndInitializeGroup"; const res = await base44.functions.invoke(functionName, { quote_id: quote.id }); if (!res.data?.success) return toast.error("אישור ההצעה נכשל"); } refresh(); toast.success(action === "reject" ? "ההצעה נדחתה ונשמרה בהיסטוריה" : "ההצעה אושרה והקבוצה הופעלה"); };
