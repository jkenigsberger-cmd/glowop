...
export default function Quotes() {
  ...
  const decide = async (quote, action, selectedOptionKey = "A") => {
    ...
    if (!res.data?.success) return toast.error("אישור ההצעה נכשל");
    ...
    refresh(groupId); toast.success("ההצעה אושרה והקבוצה הופעלה");
  };
  const duplicateQuote = async (quote) => { const res = await base44.functions.invoke("manageQuoteOptions", { action: "duplicate_quote", quote_id: quote.id }); if (res.data?.success) { refresh(quote.group_id); toast.success("הצעת המחיר שוכפלה"); } else toast.error("שכפול הצעת המחיר נכשל"); };
